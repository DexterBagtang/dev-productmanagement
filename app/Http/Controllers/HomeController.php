...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
      ...
      $showCounts2 = Salesrequest::where('status','PM Check Contractor Cost')->count();

      $showCounts3 = Salesrequest::where('status','Revenue Mark Up')
      ->orwhere('status','Revenue Re-Mark Up')->count();
      $showCounts4 = Salesrequest::where('status','PM Mark Up Technical Check')->count();

      $showCounts5 = Salesrequest::where('status','Purchasing Bidding')
      ->orwhere('status','like',"Purchasing Rebidding%")->count();

      $showCounts6 = Salesrequest::where('status','PM Designing')
      ->orwhere('status','PM -> Redesign')->count();

      $showCounts7 = Salesrequest::where('status','Revenue Head Unit for Checking')->count();

      $showCounts8 = Salesrequest::where('status','Finance Head Mark Up Approval')
          ->orwhere('status','Finance Head Mark Up Approval(Revision)')->count();

      $showCounts9 = Salesrequest::where('status','Sales Proposal Status')->count();

      $showCounts10 = Salesrequest::where('status','PM Review of Design')->count();
      $showCounts11 = Salesrequest::where('status','Sales Releasing of Proposal')->count();
      $showCounts12 = Salesrequest::where('status','Revenue Upload Contractor Bid Summary')->count();
